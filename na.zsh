...
: ${NA_DONE_TAG:="@done"}
: ${NA_MAX_DEPTH:=3}
: ${NA_AUTO_LIST_FOR_DIR:=0} # 0 to disable
...
_na_fix_output() {
  /usr/bin/ruby <<SCRIPTTIME
    ...
    input.split("\n").each {|line|
      ...
      extre = "\.$NA_TODO_EXT"
      puts "$BLUE#{base}#{filename.gsub(/#{extre}:$/,'')} $GREEN#{task.gsub(/^[ \t\-]+/,'').gsub(/ $taskTag/,'').gsub(/(@[^\(]*)((\()([^\)]*)(\)))/,"$CYAN\\\1\\\3$YELLOW\\\4$CYAN\\\5$GREEN")}"
